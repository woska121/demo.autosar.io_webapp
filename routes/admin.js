...
router.get('/memberCheck', adminRequired, function(req, res){
    ...
    UserModel.count({'authority' : false, 'state' : true}, function(err, totalCount){
        if(err) throw err;        
        var maxPageInSet = 5,   //페이지 카운트 갯수
            maxEntityInPage = 10,   // 한 페이지당 컨텐츠 수
            skipPage = (page-1) * maxEntityInPage; // skip -> 2페이지의 경우 앞의 10개 페이지 스킵, 3페이지는 20개 스킵 설정
        ...
        UserModel.find({'authority' : false, 'state' : true}).sort({date:-1}).skip(skipPage).limit(maxEntityInPage).exec(function(err, user) {
            if(err) throw err;
...
router.get('/withdrawMemberManage', adminRequired, function(req, res) {
    ...
    UserModel.count({'authority' : false}, function(err, totalCount){
        if(err) throw err;        
        var maxPageInSet = 5,   //페이지 카운트 갯수
            maxEntityInPage = 10,   // 한 페이지당 컨텐츠 수
            skipPage = (page-1) * maxEntityInPage; // skip -> 2페이지의 경우 앞의 10개 페이지 스킵, 3페이지는 20개 스킵 설정
        ...
        UserModel.find({'authority' : false, 'state' : false}).sort({date:-1}).skip(skipPage).limit(maxEntityInPage).exec(function(err, user) {
            if(err) throw err;
...
router.get('/dormancyMemberManage', adminRequired, function(req, res) {
   ...
   UserModel.count({'authority' : false}, function(err, totalCount){
        ...
        UserModel.find({'authority' : false, 'dormant_state' : true}).sort({date:-1}).skip(skipPage).limit(maxEntityInPage).exec(function(err, final_result) {
            if(err) throw err;
